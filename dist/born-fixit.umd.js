(function (global, factory) {
  ...
  var FixIt = /*#__PURE__*/function () {
    function FixIt() {
      ...
      this.target = (typeof this.options.target === 'string' ? document.querySelector(this.options.target) : this.options.target) || false; // this.offsetElements = false;
      ...
      this._scrollDirectionWait = this.options.scrollDirectionWait || 100; //Minimum distance to scroll within the "wait" period.

      this._scrollPositionThereshold = this.options.scrollPositionThereshold || 75; //Amount of times the scroll should fire before allowing to change the direction.
    ...
    _createClass(FixIt, [{
      ...
      value: function updateStickyStatus(isAutoUpdate) {
        ...
        isAutoUpdate = typeof isAutoUpdate === 'boolean' ? isAutoUpdate : false; //Indicates if the FixIt element has changed positions and prevents making unnecessary recalculations.
        ...
        if (!isAutoUpdate || isAutoUpdate && this._previousDocumentHeight !== this.getDocumentHeight()) {
          this.setRectangles(); //Set a property for the directional start location depending on wether or not `fromViewportBottom` is set to TRUE.

          this._scrollListeningStart = this.options.fromViewportBottom ? this._placeholderRect.top + this._targetRect.height - Math.max(window.innerHeight, document.documentElement.clientHeight) : this._placeholderRect.top; //The first portion of the following conditional checks if the target is smaller than its parent.
          ...
          if ((!this.parentContainer || this._targetRect.height < this._parentContainerRect.height) && this._scrollListeningStart < this.offset) {
            ...
            this.toggletFullyScrolled(this._placeholderRect.top + this._targetRect.height < this.offset); //Only request to change the direction if this flag is turn on.
      ...
      value: function containInParent() {
        if (this.parentContainer && this.isActive) {
          ...
          this.setRectangles(); //Make sure bottom of parent is visible, then ensure the target and the parent's bottom are at the same level, then confirm the window's offset is not over the target
      ...
      * Removes the placeholder element.
      ...
      value: function requestScrollDirectionUpdate(newScrollDirection) {
        ...
        this._newScrollPosition = this._placeholderRect.top; //Throttle how often the scroll direction change should be called.
        ...
        if (this._setScrollDirectionCallCount >= this._scrollDirectionThrottle) {
          this._updateScrollDirectionOnThreshold(newScrollDirection); //Reset the call count once it has reached the minimum threshold.


          this._setScrollDirectionCallCount = 0;
        }

        window.clearTimeout(this._scrollDirectionTimeout); //Set a timeout to ensure that the last position is stored after the user stops scrolling.
      ...
      value: function updateScrollDirection(newScrollDirection) {
        ...
        if (this._prevScrollDirection !== newScrollDirection) {
          ...
          this.target.classList.remove("fixit--scroll-".concat(this._prevScrollDirection)); //Attach a special class when the direction has changed at least once.
